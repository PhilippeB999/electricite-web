...
const STORAGE_KEY = "electricitequest_state_v1";
...
const ACCESS_CODE = "ELEC-2026-HDB7"; // code fourni au centre de formation après achat de la licence
...
const SYNC_KEY = "electricitequest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("electricitequest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("electricitequest_device_id", id);
  }
  return id;
}
